...
install: $(NAMESPACE)
  ...
  # use mkdir vs. install -D/d (macos portability)
	@mkdir -p $(bindir)
	@install bin/dm $(bindir)/dm
	@install bin/dmc $(bindir)/dmc
	...
	@( \
	  ...
	  for file in dm; do sed \
	      -e '/\@start/,/\@end/d' \
		    ...
		    $(CWD)/$$file > $(CWD)/bin/$$file ; \
	    find $(CWD)/lib.d/ -type f -name "*.sh" -exec cat {} >> $(CWD)/bin/$$file + ; \
	    chmod +x $(CWD)/bin/$$file ; \
		  echo "  + built $(CWD)/bin/$$file" ; \
		done ; \
	)
...
GH_PROJECT:=BlueAcornInc/devops-bootstrap
...
_mkrelease: _release_check $(NAMESPACE)
	...
	@( \
	  echo "  * attempting to create release $(RELEASE_TAG) ..." ; \
		...
		echo "  * uploading $(CWD)/bin/dm and $(CWD)/bin/dmc to release $(RELEASE_TAG) ($$id) ..." ; \
    curl -sL -H "Authorization: token $(GH_TOKEN)" -H "Content-Type: text/x-shellscript" --data-binary @"$(CWD)/bin/dm" -X POST $(GH_UPLOAD_URL)/repos/$(GH_PROJECT)/releases/$$id/assets?name=dm &>/dev/null ; \
		curl -sL -H "Authorization: token $(GH_TOKEN)" -H "Content-Type: text/x-shellscript" --data-binary @"$(CWD)/bin/dmc" -X POST $(GH_UPLOAD_URL)/repos/$(GH_PROJECT)/releases/$$id/assets?name=dmc &>/dev/null ; \
	)
